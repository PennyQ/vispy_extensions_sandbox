...
theta = 0.
phi = 0.
# if the timer is checked, maybe add a checkbox in Glue UI
checked = True
transform = MatrixTransform()
transform.translate((64, 64, 0))
...
def rotate(event):
    global theta, phi, transform
    theta += speed
    phi += speed
    transform.reset()
    # transform.rotate(theta, (0, 0, 1))
    transform.rotate(phi, (1, 0, 0))
    # transform.scale((100, 100, 0.001))
    transform.translate([64, 64, 0])
    # canvas.update()
    
timer = app.Timer(connect=rotate)
timer.start(0.01) if checked else timer.stop()
